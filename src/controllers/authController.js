...
const generateTokens = (user, tokenFamily = null) => {
  ...
  const refreshToken = jwt.sign(
    ...
    { expiresIn: authConfig.jwt.refreshExpiresIn, issuer: authConfig.jwt.issuer }
  );
...
export const register = async (req, res) => {
  ...
  try {
    ...
    if (passwordErrors.length > 0) {
      return res.status(400).json({ status: false, message: passwordErrors[0] });
    }
    ...
    const user = await prisma.user.create({
      data: { email, password: hashedPassword, firstName, lastName, phone, roleId },
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      await activityLogService.log({ action: 'login', module: 'auth', ipAddress, userAgent, status: 'failure', errorMessage: 'User not found' });
    ...
    if (lockStatus.locked) {
      return res.status(423).json({ status: false, message: 'Account is locked', lockedUntil: lockStatus.lockedUntil });
    }
    ...
    if (!isValidPassword) {
      const result = await securityService.checkAndHandleFailedLogin(user, ipAddress, userAgent);
      await activityLogService.logLogin(user.id, ipAddress, userAgent, 'failed', 'Invalid password');
      
      if (result.locked) {
        // Send email notification about account lock
        await emailService.sendSecurityAlertEmail(user.email, 'account_locked', { ipAddress, device: userAgent }, user.firstName);
        return res.status(423).json({ status: false, message: 'Account locked due to too many failed attempts', lockedUntil: result.lockedUntil });
      }
      return res.status(401).json({ status: false, message: 'Invalid credentials' });
    }
...
export const refreshToken = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.status !== 'active') {
      return res.status(401).json({ status: false, message: 'User not found or inactive' });
    }
    ...
    // Generate new tokens
    const { accessToken, refreshToken: newRefreshToken, family } = generateTokens(user, decoded.family);
    ...
    res.json({ status: true, data: { accessToken, refreshToken: newRefreshToken } });
  } catch (error) {
...
export const logoutAllDevices = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ status: false, message: 'Failed to logout from all devices' });
  }
};
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    await prisma.user.update({
      where: { id: userId },
      data: { password: hashedPassword, passwordChangedAt: new Date(), mustChangePassword: false },
    });

    // Send notification email
    await emailService.sendSecurityAlertEmail(user.email, 'password_changed', { ipAddress, device: userAgent }, user.firstName);
...
export const getActiveSessions = async (req, res) => {
  try {
    const sessions = await prisma.session.findMany({
      ...
      select: { id: true, ipAddress: true, userAgent: true, deviceInfo: true, createdAt: true, lastActivityAt: true },
...
export const terminateSession = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ status: false, message: 'Failed to terminate session' });
  }
};

export const getLoginHistory = async (req, res) => {
  try {
    const history = await activityLogService.getLoginHistory(req.user.userId, 20);
    res.json({ status: true, data: history });
  } catch (error) {
    console.error('Get login history error:', error);
    res.status(500).json({ status: false, message: 'Failed to get login history' });
  }
};
...
export const getAllActivityLogs = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ status: false, message: 'Failed to get activity logs' });
  }
};
...
export const getAllUsers = async (req, res) => {
  ...
  try {
    ...
    res.json({ status: true, data: { users, total, page: parseInt(page), limit: parseInt(limit), totalPages: Math.ceil(total / parseInt(limit)) } });
  } catch (error) {
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    await activityLogService.log({
      ...
      oldValues: { firstName: oldUser.firstName, lastName: oldUser.lastName, status: oldUser.status, roleId: oldUser.roleId },
...
export const updateRole = async (req, res) => {
  ...
  try {
    const oldRole = await prisma.role.findUnique({ where: { id: parseInt(id) }, include: { permissions: true } });
    if (!oldRole) {
      return res.status(404).json({ status: false, message: 'Role not found' });
    }

    if (oldRole.isSystem) {
      return res.status(403).json({ status: false, message: 'Cannot modify system role' });
    }
...
export const deleteRole = async (req, res) => {
  ...
  try {
    ...
    if (role.isSystem) {
      return res.status(403).json({ status: false, message: 'Cannot delete system role' });
    }
...
export const getPermissions = async (req, res) => {
  try {
    ...
    res.status(500).json({ status: false, message: 'Failed to get permissions' });
  }
};
